...
public class Transaksi {
    ...
    public void prosesTransaksi(Member member, Transaksi transaksi, Barang barang){
        ...
        for (int j = 0; j < x; j++){
            ...
            total+=jumlah;
            System.out.println(barang.getNamaBarang(idBarang.get(j))+"\t" + idBarang.get(j)+"\t" + barang.getHarga(idBarang.get(j))+"\t" + jumlah); transaksi.setTransaksi(barang, idMember, idBarang.get(j), banyak.get(j));
        }
